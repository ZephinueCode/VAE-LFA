...
ULTRAEDIT_DIFFUSERS_ALIAS = "ultraedit_diffusers"
...
class SD3Runner:
    def __init__(
        ...
        self.sd3_pipeline = str(config.model.sd3_pipeline).strip().lower()
        if self.sd3_pipeline not in {"img2img", "ultraedit"}:
            raise ValueError(
                f"Unsupported model.sd3_pipeline={config.model.sd3_pipeline!r}. "
                "Expected 'img2img' or 'ultraedit'."
            )
        if self.sd3_pipeline == "ultraedit" and self.use_lpt:
            raise ValueError("UltraEdit mode does not support --latent true yet. Please use --latent false.")
        ...
        self._supports_mask_key = self._detect_mask_key(self._call_signature)
    ...
    def _load_pipeline(self, config: AppConfig) -> Any:
        ...
        if self.sd3_pipeline == "ultraedit":
            StableDiffusion3InstructPix2PixPipeline = self._load_ultraedit_pipeline_class()
            pipe = StableDiffusion3InstructPix2PixPipeline.from_pretrained(config.model.model_path, **kwargs)
        else:
            pipe = StableDiffusion3Img2ImgPipeline.from_pretrained(config.model.model_path, **kwargs)
    ...
    def _load_ultraedit_pipeline_class(self) -> type[Any]:
        package = self._load_ultraedit_diffusers_package()
        module = importlib.import_module(
            f"{ULTRAEDIT_DIFFUSERS_ALIAS}.pipelines.stable_diffusion_3"
        )
        pipeline_cls = getattr(module, "StableDiffusion3InstructPix2PixPipeline", None)
        if pipeline_cls is None:
            raise ImportError(
                "Vendored UltraEdit diffusers package was loaded, but "
                "StableDiffusion3InstructPix2PixPipeline was not found."
            )
        return pipeline_cls

    def _load_ultraedit_diffusers_package(self) -> Any:
        if ULTRAEDIT_DIFFUSERS_ALIAS in sys.modules:
            return sys.modules[ULTRAEDIT_DIFFUSERS_ALIAS]

        package_root = Path(self.config.model.sd3_ultraedit_diffusers_path).expanduser().resolve()
        init_path = package_root / "__init__.py"
        if not init_path.exists():
            raise FileNotFoundError(
                "UltraEdit vendored diffusers package was not found at "
                f"{package_root}. Expected __init__.py there."
            )

        spec = importlib.util.spec_from_file_location(
            ULTRAEDIT_DIFFUSERS_ALIAS,
            init_path,
            submodule_search_locations=[str(package_root)],
        )
        if spec is None or spec.loader is None:
            raise ImportError(f"Unable to load UltraEdit diffusers package from {package_root}.")

        module = importlib.util.module_from_spec(spec)
        sys.modules[ULTRAEDIT_DIFFUSERS_ALIAS] = module
    ...
    @staticmethod
    def _detect_mask_key(signature: inspect.Signature) -> str | None:
        for key in ("mask_img", "mask_image", "mask"):
            if key in signature.parameters:
                return key
        return None
    ...
    def _encode_context(self, current_latents: torch.Tensor | None):
        ...
        orig_preprocess = self.pipe.image_processor.preprocess
        ...
        def patched_preprocess(image, height=None, width=None):
            if isinstance(image, torch.Tensor) and image.shape[1] == self.pipe.vae.config.latent_channels:
                return image
            return orig_preprocess(image, height=height, width=width)

        self.pipe.vae.encode = patched_encode
        self.pipe.image_processor.preprocess = patched_preprocess
        try:
            yield
        finally:
            self.pipe.vae.encode = orig_encode
            self.pipe.image_processor.preprocess = orig_preprocess
    ...
    def _run_round(
        ...
        *,
        ...
        height: int,
        width: int,
    ) -> tuple[torch.Tensor | None, Image.Image]:
        if self.sd3_pipeline == "ultraedit":
            return self._run_round_ultraedit(
                prompt=prompt,
        ...
        return self._run_round_img2img(
            prompt=prompt,
            current_image_pil=current_image_pil,
    ...
    def _run_round_img2img(
        ...
        *,
        ...
        if self._supports_image_key is None:
            raise RuntimeError("Current StableDiffusion3Img2ImgPipeline call signature does not support image-to-image input.")

        if self.use_lpt and current_latents is not None:
            kwargs[self._supports_image_key] = current_latents
        else:
    ...
    def _run_round_ultraedit(
        ...
        *,
        ...
        height: int,
        width: int,
    ) -> tuple[None, Image.Image]:
        if self._supports_image_key is None:
            raise RuntimeError("Current SD3 UltraEdit pipeline does not expose an image input key.")
        if self._supports_mask_key is None:
            raise RuntimeError("Current SD3 UltraEdit pipeline does not expose a mask input key.")
        if not self.config.inference.free_form_mask:
            raise ValueError("Only free-form UltraEdit is supported right now. Set inference.free_form_mask: true.")

        resized_image = current_image_pil.resize((width, height), resample=Image.Resampling.LANCZOS)
        mask_image = Image.new("RGB", resized_image.size, (255, 255, 255))
        kwargs: dict[str, Any] = {
            "prompt": prompt,
            self._supports_image_key: resized_image,
            self._supports_mask_key: mask_image,
        ...
        if isinstance(images, (list, tuple)):
            ...
            vis_image = images
        if not isinstance(vis_image, Image.Image):
            raise RuntimeError("UltraEdit pipeline did not return a PIL image.")
